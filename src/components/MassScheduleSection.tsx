...
export function MassScheduleSection() {
  ...
  return (
    <section id="horarios" className="bg-ice py-20">
          ...
          {massSchedule.map((schedule) => (
            ...
            >
              <CardContent className="p-6">
                <h3 className="mb-3 text-lg font-semibold text-navy group-hover:text-royal">
                  {schedule.day}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {schedule.times.map((time) => (
                    <span
                      key={time}
                      className="inline-flex items-center rounded-full bg-secondary px-3 py-1 text-sm font-medium text-navy"
                    >
                      {time}
                    </span>
                  ))}
